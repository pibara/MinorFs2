...
void *capfs_init(struct fuse_conn_info *conn) {
   return reinterpret_cast<void*>(new CapFsGuard());
}
void capfs_destroy(void *capfsgvoid){
  CapFsGuard *capfsguard=reinterpret_cast<CapFsGuard *>(capfsgvoid);
  delete capfsguard;
}
...
int capfs_getattr(const char *p, struct stat *s) {
  ...
  CapFsGuard *capfsguard=reinterpret_cast<CapFsGuard *>(context->private_data);
  CapFs &capfs= (*capfsguard)(context->gid,context->pid); 
...
int main(int argc, char *argv[])
{
  ...
  fuseargv[fuseargcount+3]="/mnt/capfs";
